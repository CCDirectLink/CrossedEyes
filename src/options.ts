...
export class MenuOptions implements InitPoststart {
    static get spacialAudioEnabled(): boolean {
        return opt(spacialAudioToggleId)
    }
    static get loudWallsEnabled(): boolean {
        return opt(loudWallsToggleId) && MenuOptions.spacialAudioEnabled
    }
    static get loudEntitiesEnabled(): boolean {
        return opt(loudEntitiesToogleId) && MenuOptions.spacialAudioEnabled
    }
    static get puzzleEnabled(): boolean {
        return opt(puzzleToggleId) && MenuOptions.spacialAudioEnabled
    }
    static get footstepVolume(): number {
        return opt(footstepVolumeId)
    }
    static get textBeeping(): number {
        return opt(textBeepingId)
    }

    static get ttsEnabled(): boolean {
        return opt(ttsToogleId)
    }
    static get ttsCharEnabled(): boolean {
        return opt(ttsCharToogleId) && MenuOptions.ttsEnabled
    }
    static get ttsMenuEnabled(): boolean {
        return opt(ttsMenuToogleId) && MenuOptions.ttsEnabled
    }
    static get ttsType(): TTSTypes {
        return opt(ttsTypeId)
    }
    static set ttsType(v: TTSTypes) {
        sopt(ttsTypeId, v)
    }
    static get ttsSpeed(): number {
        return opt(ttsSpeedId)
    }
    static get ttsVolume(): number {
        return opt(ttsVoulmeId)
    }
    static get ttsPitch(): number {
        return opt(ttsPitchId)
    }
    ...
    constructor() {
        ...
        sc.OPTIONS_DEFINITION[ttsTypeId] = {
            ...
            data: Object.entries(TTSTypes)
                .splice(Object.keys(TTSTypes).length / 2)
                .reduce(
                    (acc, [k, _], i) => {
                        if (typeof k === 'string') {
                            acc[k] = i
